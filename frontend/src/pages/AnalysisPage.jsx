/**
 * AnalysisPage.jsx - Versão Final com Análise Sequencial Avançada
 * 
 * Inclui todas as funcionalidades:
 * - Análise de Gatilhos
 * - Análise Preditiva
 * - Análise Sequencial Avançada (NOVO)
 * - Análise Histórica
 * - Lista de Jogos
 */
...
export default function AnalysisPage() {
  ...
  const [lookbackGames, setLookbackGames] = useState(20); // NOVO: para análise sequencial
  ...
  const [sequentialAnalysis, setSequentialAnalysis] = useState(null); // NOVO
  ...
  const handlePredictiveAnalysis = async () => {
    ...
    try {
      ...
      const errorMsg = 'Erro ao analisar padrões preditivos: ' + (err.response?.data?.detail || err.message);
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  // NOVO: Análise Sequencial Avançada
  const handleSequentialAnalysis = async () => {
    if (!selectedMarket) {
      setError('Por favor, selecione um mercado');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/advanced-analysis/sequential-patterns`, {
        target_market: selectedMarket,
        reference_date: referenceDate,
        lookback_games: parseInt(lookbackGames)
      });
      
      setSequentialAnalysis(response.data);
      setActiveTab('sequential');
    } catch (err) {
      const errorMsg = 'Erro ao analisar padrões sequenciais: ' + (err.response?.data?.detail || err.message);
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={handleSequentialAnalysis}
                disabled={!dbStatus.connected || loading || !selectedMarket}
                className="w-full px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors text-sm"
              >
                {loading && activeTab === 'sequential' ? 'Analisando...' : 'Análise Sequencial'}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="p-6">
            ...
            {/* Tab: Análise Sequencial Avançada (NOVO) */}
            {activeTab === 'sequential' && (
              <AdvancedSequentialAnalysisTab
                analysisData={sequentialAnalysis}
                loading={loading}
                onAnalyze={handleSequentialAnalysis}
                selectedMarket={selectedMarket}
                dbConnected={dbStatus.connected}
                lookbackGames={lookbackGames}
                setLookbackGames={setLookbackGames}
              />
            )}
